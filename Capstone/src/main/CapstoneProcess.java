...
public class CapstoneProcess {
	...
	public void fileAccidentClaims() {
		...
		else if(policyList.stream().filter(a->((Policy) a).getPolicyNumber().equalsIgnoreCase(policyNumber) && ((Policy) a).getIsExpired()==1).toList().size()>=1) {
			print.headerLabel("CANT FILE CLAIM POLICY IS ALREADY CANCELED");
		}
	...
	public void searchClaims () {
		...
		// check iof claim number exist
		if(claimList.stream().filter(a->((Claims) a).getClaimNumber().equalsIgnoreCase(claimNumber) ).toList().size()>=1) {
			
			// iterte the claimList to get the need value base on filtration 
			for(Object claimItem:claimList.stream().filter(a->((Claims) a).getClaimNumber().equalsIgnoreCase(claimNumber) ).toList()) {
				// print claim policy customer account and policy holder of the claim number

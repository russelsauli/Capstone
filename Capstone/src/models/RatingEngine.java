...
public class RatingEngine extends Vehicle{
	...
	public double ratingEngine(Vehicle vehicle) {	
		...
		dlx = Integer.parseInt(vehicle.getDriversLicenseIssued().substring(0, vehicle.getDriversLicenseIssued().indexOf("-")));	// substirng or getting  only the year of the license issued date using substring method and index of 
		...
		vehicle.setPremiumCharged((vehicle.getPurchasePrice() * vpf) + ((vehicle.getPurchasePrice() / 100) / dlx));	// set premiumcharge;
		
		vehicle.setPremiumCharged(Math.round(vehicle.getPremiumCharged() * 100.0) / 100.0); // set premiumcharge 2 deciamal point;

...
public class CapstoneSubProcess {
	...
	public Vehicle vehicleData() {
		...
		do {
			...
			if ((LocalDate.now().getYear() - tempVehicle.getYear()) > 40) {	// if vehicle is more than or equals 40 years
		...
		do {
			// set value for type of car
			selectionChoiceCar = inputValidator.returnInterger("Select the type of car\n1: 4-door sedan\n2: 2-door sports car\n3: SUV\n4: Truck\nSelect: ");
		...
		do {
			// set  fuel type
			selectionChoiceCar = inputValidator.returnInterger("Select the FUEL type of car\n1: Diesel\n2: Electric\n3: Petrol\nSelect: ");
	...
	public boolean isVehicleExist(ArrayList<Vehicle> vehicleList, String PlateNumber) {
		...
		for (Vehicle tempVehicle : vehicleList) {
			...
			if (tempVehicle.getPlateNumber().equalsIgnoreCase(PlateNumber)) {
				...
				isVehicleExist = true;	// if vehicle exist change value
			}
		}
	...
	// a method that set value of accident claims
	public Claims claimDetail(Policy policy) {
		...
		do {
			...
			if(LocalDate.parse(accidentDate).isBefore(LocalDate.parse(policy.getEffectiveDate()))) {	// if set claim date is before the effective date 
				...
				// loop until dclaim number not existing in database
				do{
					
					claimNumber=generateNumber(5,99999,"C");	// generate claim number
					
					claimlist=claims.loadData();	// reload the data of claim list
